...
describe('NIS key pair', () => {
    describe('sign', () => {
        ...
        it('returns same signature for same data signed by same key pairs', () => {
            ...
            const keyPair1 = new Nis1KeyPair(Key.createFromHex(privateKey));
            const keyPair2 = new Nis1KeyPair(Key.createFromHex(privateKey));
            const payload = crypto.randomBytes(100);
        ...
        it('returns different signature for same data signed by different key pairs', () => {
            // Arrange:
            const keyPair1 = Nis1KeyPair.generate();
            const keyPair2 = Nis1KeyPair.generate();
            const payload = crypto.randomBytes(100);
    ...
    describe('verify', () => {
        ...
        it('fails if public key does not correspond to private key', () => {
            ...
            // Act:
            const isVerified = Nis1KeyPair.generate().verify(payload, signature);
    ...
    describe('generate', () => {
        it('Can generate a random keypair', () => {
            // Act:
            const key = Nis1KeyPair.generate();
