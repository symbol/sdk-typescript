...
describe('base32', () => {
    ...
    describe('encode', () => {
        ...
        it('can convert test vectors', () => {
            ...
            for (const sample of Test_Vectors) {
                const input = Converter.hexToUint8(sample.decoded);
    ...
    describe('decode', () => {
        it('can convert empty input', () => {
            ...
            // Assert:
            expect(Converter.uint8ToHex(decoded)).to.equal('');
        });
        ...
        it('can convert test vectors', () => {
            ...
            for (const sample of Test_Vectors) {
                ...
                // Assert:
                expect(Converter.uint8ToHex(decoded), `input ${sample.encoded}`).to.equal(sample.decoded);
            }
        });
        ...
        it('accepts all valid characters', () => {
            ...
            // Assert:
            expect(Converter.uint8ToHex(decoded)).to.equal('00443214C74254B635CF84653A56D7C675BE77DF');
        });
    ...
    describe('roundtrip', () => {
        ...
        it('encode -> decode', () => {
            ...
            for (const input of inputs) {
                // Act:
                const encoded = Base32.Base32Encode(Converter.hexToUint8(input));
                const result = Base32.Base32Decode(encoded);

                // Assert:
                expect(Converter.uint8ToHex(result), `input ${input}`).to.equal(input);
            }
        });
    });
});
