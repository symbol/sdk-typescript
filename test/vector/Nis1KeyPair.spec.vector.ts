...
describe('nis1 key pair - TestVector', () => {
    describe('construction', () => {
        it('can extract from private key test vectors', (done) => {
            const stream = fs.createReadStream(path.join(__dirname, '../test-vector/nis1/1.test-keys.json'), { encoding: 'utf-8' });
            stream.pipe(
    ...
    describe('sign & verify- Test Vector', () => {
        it('sign', (done) => {
            const stream = fs.createReadStream(path.join(__dirname, '../test-vector/nis1/2.test-sign.json'), { encoding: 'utf-8' });
            stream.pipe(
                JSONStream.parse([]).on('data', (data) => {
                    data.forEach((s) => {
                        // Arrange:
